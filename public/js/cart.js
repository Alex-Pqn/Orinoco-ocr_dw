...
createElement = (name, lenses, price, index) => {
  ...
  nameElement.className =
    'offset-lg-1 col-lg-2 col-md-3 col-sm-3 col-3 text-left';
  lensesElement.className =
    'offset-lg-1 col-lg-3 col-md-4 col-sm-5 col-6 text-left';
...
formExpressionValidation = (value) => {
  let expressionValidation = {
    ...
    regexEmail:
      /^([\w-]+(?:\.[\w-]+)*)@((?:[\w-]+\.)*\w[\w-]{0,66})\.([a-z]{2,6}(?:\.[a-z]{2})?)$/, //e-mail validation
    regexCaracters:
      /\`|\~|\!|\@|\#|\$|\%|\^|\&|\*|\(|\)|\+|\=|\[|\{|\]|\}|\||\\|\'|\<|\,|\.|\>|\?|\/|\""|\;|\:|\s/, //specials caracters
...
formValidation = () => {
  expressionValidationList.forEach((expression) => {
    ...
    if (expression.value.length < expression.minLength) {
      ...
      displayErrorForm(
        'Vous devez entrer au minimum ' + expression.minLength + ' caractères.',
        expression
      );
    } else if (expression.value.length > expression.maxLength) {
      event.preventDefault();
      displayErrorForm(
        "Vous ne pouvez entrer qu'au maximum " +
          expression.maxLength +
          ' caractères.',
        expression
      );
    } else if (
      expressionsList.regexContainsNumber === expression.regexContainsNumber
    ) {
      event.preventDefault();
      ...
      if (
        expressionValidationList[0].stepValidation === true &&
...
postMethod = (products, contact) => {
  ...
  xhr.onerror = () => {
    ...
    console.error(
      'Résultat de requête API & Statut HTTP : ' +
        this.status +
        ', état readyState : ' +
        this.readyState
    );
...
cartDisplay = () => {
  if (responseTextAPI != undefined) {
    //if there is an order waiting to be "accepted", redirect to "orderConfirmation" page
    window.location.replace('orderConfirmation.html');
  } else if (orders == undefined) {
    //if there is nothing products in the cart, display error page
    errorPageEmptyCart();
  } else {
    //if there is at least one element in localStorage "orders" array (= minimum one product in cart)
    const submitCartButton = document.querySelector(
      '.cart__contact__form__btn-submit input'
    );

    for (let i = 0; i < orders.length; i++) {
      createElement(orders[i].name, orders[i].lenses, orders[i].price, i);
    ...
    submitCartButton.addEventListener('click', (event) => {
      //on click "Valider ma commande" button, the functions for the verifications are called
      storeValuesForm();
      formValidation();
    });
  }
