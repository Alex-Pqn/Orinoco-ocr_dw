...
pageOrderProduct = (imageUrl, name, description, price) => {
  ...
  prdctImageUrl.setAttribute(
    'aria-label',
    'Appareil photographique ' + name + ' du site Orinoco'
  );
  prdctButton.setAttribute(
    'aria-label',
    "Ajouter au panier l'appareil " + name
  );
...
displayCustomListSelect = (lensesOptions) => {
  lensesOptions.forEach((lensesOptions) => {
    //browse all product choices
    ...
    custom.setAttribute(
      'aria-label',
      'Liste déroulante pour personnaliser le produit'
    );
    ...
    option.setAttribute(
      'aria-label',
      'Sélectionner la personnalisation ' + lensesOptions
    );
...
const apiProducts = async () => {
  ...
  xhr.onreadystatechange = function () {
    const apiStatusReady =
      this.readyState === 4 && this.status === 200 && xhr.DONE; //status when API is ready
    ...
    if (apiStatusReady) {
      ...
      try {
        //recover url "?id=" parameter and store in const "id"
        ...
        for (let i = 0; i < cameras.length; i++) {
          if (id === cameras[i]._id) {
            //if an id in the API is matched with the id in the url parameter
            foundUrl = true;
            pageOrderProduct(
              cameras[i].imageUrl,
              cameras[i].name,
            ...
            displayCustomListSelect(cameras[i].lenses);
            customChoiceListener(cameras[i].lenses[0]);
            finalisationButton(
              cameras[i].name,
              cameras[i].price,
              cameras[i]._id
        ...
        if (!foundUrl) {
          //if no API id matched with the id in the url parameter, redirect to home
          window.location.replace('../../index.html');
        }
      } catch (err) {
        //if the url parameter can't be create, redirect to home
        console.error(
          "Erreur rencontrée lors de la création du paramètre d'url : " + err
        );
        window.location.replace('../../index.html');
      }
    } else if (apiStatusNotReady) {
      //if API is not ready, return errors + status readyState & http in console
      displayErrorPageAPI();
      console.error('La requête GET en direction de ' + url + ' a échouée.');
      console.error(
        'Résultat de requête API / Statut HTTP : ' +
